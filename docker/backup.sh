#!/bin/bash
# ============================================================
# AI Blog Bot — Database Backup Script
# ============================================================
# Usage:
...
BACKUP_FILE="${BACKUP_DIR}/ai_blog_bot_${TIMESTAMP}.sql.gz"
...
if ! docker ps --format '{{.Names}}' | grep -q "ai-blog-bot-db"; then
  echo -e "${RED}[ERROR] MySQL container (ai-blog-bot-db) is not running${NC}"
...
docker exec ai-blog-bot-db mysqldump \
  -u "$DB_USER" \
  -p"$DB_PASS" \
  --single-transaction \
  --routines \
...
BACKUP_COUNT=$(ls -1 "${BACKUP_DIR}"/ai_blog_bot_*.sql.gz 2>/dev/null | wc -l)
if [ "$BACKUP_COUNT" -gt 10 ]; then
  REMOVE_COUNT=$((BACKUP_COUNT - 10))
  echo -e "${YELLOW}[CLEANUP] Removing ${REMOVE_COUNT} old backup(s)...${NC}"
  ls -1t "${BACKUP_DIR}"/ai_blog_bot_*.sql.gz | tail -n "$REMOVE_COUNT" | xargs rm -f
fi

echo -e "${GREEN}[DONE] Backup complete. Total backups: $(ls -1 "${BACKUP_DIR}"/ai_blog_bot_*.sql.gz 2>/dev/null | wc -l)${NC}"
